...
pub const IncrementalUtxo = struct {
    ...
    /// blocks), looking transactions up in `blocks`.
    pub fn update(self: *IncrementalUtxo, order: []const Hash256, blocks: *const BlockTxMap) !void {
        ...
        while (self.applied.items.len > d) {
            ...
            _ = self.applied.pop();
            for (bu.removed) |op| _ = self.set.spend(op);
            for (bu.restored) |c| try self.set.add(self.gpa, c.op, c.out);
            bu.deinit(self.gpa);
        }
        ...
        for (order[d..]) |id| {
            ...
            const bu = try self.applyBlock(id, txs);
    ...
    /// coinbases mint; other txs apply if valid, else are dropped), returning
    /// the undo data.
    fn applyBlock(self: *IncrementalUtxo, id: Hash256, txs: []const Transaction) !BlockUndo {
        _ = id;
        ...
        for (txs) |tx| {
            if (tx.isCoinbase()) {
                ...
                for (tx.inputs) |in| {
                    ...
                    try restored.append(self.gpa, .{ .op = in.outpoint, .out = coin });
                }
                for (tx.outputs, 0..) |o, i| {
                    const op = OutPoint{ .txid = txid, .index = @intCast(i) };
                    try self.set.add(self.gpa, op, o);
...
test "property: incremental UTXO equals recompute across random DAG growth" {
    ...
    while (seed < 3050) : (seed += 1) {
        ...
        _ = try processor.applyOrder(gpa, &rec, &gd, items.items);
